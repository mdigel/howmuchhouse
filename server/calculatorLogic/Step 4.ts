...
type StateTaxRates = { [key: string]: number };
...
interface NetIncomeAnnualStats {
  grossIncome: number;
...
function calculateNetIncome(
  grossIncome: number,
  state: keyof StateTaxRates,
  filingStatus: FilingStatus,
  pretaxContributions: number = 0,
  dependents: number = 0
): NetIncomeAnnualStats {
  // Federal tax brackets for 2024
  const federalTaxBrackets: Record<FilingStatus, TaxBracket[]> = {
    single: [
  ...
  // State income tax rates (simplified; actual tax may vary based on brackets and deductions)
  const stateTaxRates: StateTaxRates = {
    AL: 0.05,
    AK: 0.0,
    AZ: 0.025,
    AR: 0.044,
  ...
  const socialSecurityTax =
    Math.min(adjustedGrossIncome, socialSecurityWageBase) * 0.062;
  const medicareTax = adjustedGrossIncome * 0.0145;
  const additionalMedicareTax =
    adjustedGrossIncome > 250_000 ? (adjustedGrossIncome - 250_000) * 0.009 : 0;
  ...
  // Calculate tax credits (e.g., Child Tax Credit)
...
export { calculateNetIncome, NetIncomeAnnualStats, FilingStatus };